...
### 1.5 Bonus 目標層數的「貪婪與恐懼」波動 (Cashout Variance)
- **解鎖 DNA**: `Cashout_Propensity` (收手意願)
- **實作細節**: 
  - 雖然目前 `Cashout_Stop_Level` 作為絕對的收手基準，但未來可以加入隨機波動機制（例如：15% 機率提早一關收手、15% 機率多拚一關）。
  - **預期效果**：即使 DNA 設定目標為 L3 的 Agent，也不會每一把都像機器人一樣精準停在 L3，會產生 L2~L4 的人性化手感波動。
