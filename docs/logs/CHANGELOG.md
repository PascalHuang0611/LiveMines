...
## [v2.65 / V14B] - 2026-08-21
### 🔧 調整 (Changes)
- **真實開獎改以「營運日」分組**: 早上 08:00 為分界 (20260820 =
  0820 08:00 ~ 0821 08:00，凌晨 8 點前的局歸前一營運日)，
  物理批次 Round 標籤與勾選以營運日為準 (label 懸停有說明)。
- 部署開獎檔照營運日重切: 保留營運日 20260819 (878 局) 與
  20260820 (942 局)，共 1,820 局——0819 00:00~07:59 的營運日
  0818 尾巴已切除；營運日 0821 (0821 08:00 起) 尚無資料。

## [v2.64 / V14B] - 2026-08-21
### 🔄 資料更新 (Data)
- **DNA 母體: 20260816~0819 完整 4 天** (0819 補齊全天；43,145 位玩家、
  1,377,549 筆注單、3,822 局、96 小時)。每日登入率真實值 (平均 0.288)：
  登入 1 天(0.25)×37,979 / 2 天×3,987 / 3 天×886 / 全勤×293。
  VIP: V1×40,877 ~ V7×2、V8×1 (首見 V8)。CASHOUT 真實 BG 紀錄 1,999 人。
- **真實物理開獎: 僅保留 20260819~0821** (2,125 局 Approved)——DNA 觀察期
  與開獎回放期刻意錯開 (0816~19 學行為、0819~21 跑盤面)。
- 萃取腳本支援帳單日期範圍參數
  (`python extract_features_V3_realdata.py 20260816 20260819`)。

## [v2.63 / V14A+V14B] - 2026-08-19
### 🔧 調整 (Changes)
- 0819 兩版共 14 份數值表縮排修正為與 0805 完全一致
  (2 空格縮排、陣列逗號後無空白)；並移除誤入庫的解壓縮殘留資料夾。

## [v2.62 / V14A+V14B] - 2026-08-19
### 🔧 參數更新 (Config)
- **20260819 V2 (Rebased) 七表換新版** (已重排緊湊格式)：
  - 基礎倍率回到整數 2/5/10 (不再 rebase 倍率)。
  - **gridWeights 全表改為五級梯度** (免費 135/118/100/82/70、
    付費 135/118/100/82/65，含 BASE)——V4 位置權重不再是 shadow mode，
    分數離開中性帶即實際偏移閃電落點 (實測 720 局中 502 局非中性)。
  - PRT 系列付費閃電權重大翻轉 (偏向低倍組合，如 PRT1 841/50/0/109)，
    RTP 分級改由閃電組合 + V4 位置權重承擔。
  - BASE/PRT 的 expectedMainEV 統一 0.89528。
- 「V4 非中性局數」tooltip 更新：是否偏移取決於數值表 gridWeights
  (全 100 = shadow mode；帶梯度 = 實際偏移)。

## [v2.61 / V14A+V14B] - 2026-08-19
### ✨ 新增功能 (Features)
- **新增參數版本「20260819 V2 (Rebased)」**: 7 份 Rebased v2.0 數值表
  (檔名已統一為標準命名)。與 20260819 的主要差異：
  - `singleAreaBasePayouts` 由整數 2/5/10 改為 rebased 小數
    (BASE 1.9856/4.9639/9.9278，PRT 系列更低、BST 系列 ≈ 2/5/10)。
  - 免費閃電權重改用千分制且大幅偏向單擊 (BASE 920/80)；
    付費閃電組合權重整組重配 (高倍組合多數歸零)。
  - 各表 expectedMainEV 整體下修 (BASE 0.9185 → 0.8949)。
  - 風控參數與 20260819 版相同 (大戶 3000/0.30、GGR 24h/0、L1 0.5)。
### 🔧 調整 (Changes)
- 20260819 與 20260819_V2 全部 14 份數值表重排為 0805 式緊湊格式
  (陣列不換行)，文字編輯器閱讀不再逐字換行。
...
- **20260819 風控參數同步 prod_gms 新版**: U_PRT_L1 必殺機率全五關
  1.00 → **0.50**。L1 不再必殺，GGR 捷徑強制 L1 時亦僅 50% 骰中，
  介入頻率約減半。其餘參數不變。

## [v2.58 / V14A+V14B] - 2026-08-19
### 🔧 參數更新 (Config)
- **20260819 風控參數同步 prod_gms 新版** (V3 必殺機率表 levels)：
  - U_PRT_L2: 0/0.10/0.35/0.60/0.80 → **0.15/0.20/0.25/0.30/0.40**
  - U_PRT_L3: 0/0.05/0.18/0.35/0.55 → **0.10/0.15/0.20/0.25/0.35**
  - U_PRT_L4: 0/0/0.05/0.12/0.25 → **0.05/0.10/0.15/0.20/0.25**
  - L2~L4 的 BG Level 1 由 0 改為有機率、曲線放緩；L1 (全 1.0) 與
    其餘參數 (RTP/ΔRTP 門檻、大戶、GGR) 不變。
### 🖥️ 介面 (UI)
- **V3 GGR 窗口獨立顯示**: 左側面板新增「V3 GGR 窗口」列 (目前盈虧/門檻，
  虧損低於門檻時紅色警示)，與「窗口 RTP」分開——GGR 用 24,000 局專用窗，
  RTP 窗 (48,000 局) 只服務 V2 換表與 V3 RTP 條件，兩者 tooltip 已改寫明確區分。
- **V3 介入原因改為四格卡片排版** (RTP/🐋大戶/雙重/GGR)，取代原本擠在
  一行的文字，非零值上色。
