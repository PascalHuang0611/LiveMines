        ...
        <div class="bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-lg h-full flex flex-col">
            ...
            <div class="flex flex-col gap-2 mb-3 bg-gray-900 p-2 rounded-lg">
                <div class="grid gap-2" :class="$game.simulationMode === 'agentTraffic' ? 'grid-cols-2' : 'grid-cols-3'">
                    ...
                    <button @click="$game.historyFilter = 'v3'" :class="[$game.historyFilter === 'v3' ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700', { 'col-span-2': $game.simulationMode === 'manual' }]" class="px-3 py-1.5 rounded text-xs font-bold transition">🛡️ V3 介入</button>
                    <button @click="$game.historyFilter = 'v4'" :class="$game.historyFilter === 'v4' ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'" class="px-3 py-1.5 rounded text-xs font-bold transition">🎲 V4 偏移</button>
                </div>
                <!-- 新增局數範圍篩選 -->
                ...
                <div v-for="(record, index) in $game.displayHistory" :key="record.round"
                    ...
                    <div class="flex justify-between items-center mb-1">
                        ...
                        <div class="flex space-x-2">
                            ...
                            <span v-if="record.v4Weights && record.v4Weights.nonNeutral" class="text-[10px] bg-purple-600 text-white px-2 py-0.5 rounded shadow-[0_0_8px_rgba(147,51,234,0.6)]" title="此局閃電落點使用了 V4 非中性權重 (點擊查看九格權重)">🎲 V4</span>
