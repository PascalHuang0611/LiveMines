...
每次改版完成後，必須更新 UI (App.vue 內的版本號)，並在此計畫書最下方新增對應的 Changelog。
...
## Milestone 4.75：觀光客 (Casual Tourist) 流量還原 (目前已做到)

在進入 Milestone 5 前，為了解決「加上時間流後，CCU 看起來偏低」的問題，我們修改了底層的 Python DNA 萃取腳本：

- **解除最小局數限制**：不再剔除 `< 30 局` 的玩家，讓總載入 DNA 數量從 3,200 人還原至 35,630 人（原始資料庫總下注人數）。
- **K-Means 隔離保護**：在進行機器學習分群時，依然只讓 `>= 30 局` 的活躍玩家參與，避免核心特徵（如追注、縮注）被噪音數據污染。
- **專屬過客標籤**：將未滿 30 局的 32,423 位玩家強制標記為 `persona_casual_tourist` (觀光客)。
- **效果**：透過極低的 `Daily_Login_Probability`，觀光客只會在模擬器中偶爾閃現，為 CCU 曲線帶來真實的長尾毛刺效應，而不會淹沒核心玩家的表現。
...
## Milestone 5.5：VIP Segmentation Integration

- 讀取 `PlayerData.csv` 根據 `總押注額` 將玩家分為 `V1` 到 `V8`。
- 如果資料缺失則預設為 `V1`。
- 寫入 DNA JSON 提供前端讀取。
- 前端 `ActiveAgentsModal.vue` 依照層級加上不同顏色的 VIP 徽章標示 (金、紫、藍、灰)，並支援依據 VIP 等級高低排序。
- 讓所有圖表的 Persona 顏色保持一致，觀光客固定為灰色。
...
- raw betMap 轉成合法面額組合 `[5, 10, 50, 100, 500, 1000, 10000]`。
- 產生 `chipMap`。
...
## v2.11
- Implemented Milestone 4.5 (Advanced Time Distribution). Upgraded `generateDayPlan` to incorporate `Daily_Login_Probability`, `Hourly_Activity_Vector` (weighted sampling), and multi-session capabilities via `Sessions_Per_Active_Day`. Fixed logic bugs in `roundActiveCounts` estimation and `currentActiveAgents` cross-day calculation.
- Created `ActiveAgentsModal.vue` as a PM-friendly presentation tool to view active agents and their semantic DNA profiles (betting habits, cashout strategy, daily routine).

## v2.12
- Implemented Milestone 4.75 (Casual Tourist Traffic Restoration). Updated Python extraction script (`extract_features_real_V2_5levels_lightning_chipprior.py`) to retain all players regardless of bet count. Protected K-Means clustering by restricting it to active players (>= 30 bets). Assigned the remaining 32,000+ players the `persona_casual_tourist` tag to simulate long-tail, low-frequency traffic without distorting core behavioral metrics.
...
## v2.16
- Implemented Milestone 5.5: VIP Segmentation Integration. Added VIP_Group generation to the Python extraction script based on PlayerData.csv's total bet amounts. Missing data defaults to V1.
- Updated ActiveAgentsModal.vue and gameStore.js to parse and display styled VIP badges for active agents.
- Copied generated JSON to public/agents.json for default loading.

## v2.17
- UI Polish: ActiveAgentsModal list is now sorted by VIP level (descending), with casual tourists pinned to the bottom.
- UI Polish: VIP badges moved next to Persona Names for better readability.
- UI Polish: Persona Name colors in the lists now dynamically match the colors assigned in the Persona Distribution chart. Casual tourist dots and text are explicitly forced to gray #9ca3af.

## v2.18
- Implemented Milestone 6: Agent Decision - Lightning / Cashout / Chip Legalize.
- Added decideLightning logic based on Buy_Lightning_Prob.
- Added decideCashoutStrategy logic based on Cashout_Propensity and Cashout_Stop_Level (clamped to 1~5).
